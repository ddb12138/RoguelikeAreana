...
火焰剑使用 [`resource/buffs/sword_fire.tres`](../resource/buffs/sword_fire.tres) 配置灼烧参数。在 Inspector 打开该资源即可调整：
...
选择 `剑:火焰附魔` 后，升级池会加入三条各 5 级的火焰强化：`剑:火焰伤害升级` 每级增加 1 点每跳伤害，`剑:火焰持续升级` 每级增加 1 秒持续时间，`剑:火焰频率升级` 每级缩短 0.2 秒灼烧间隔，最低间隔为 0.2 秒。三者只影响之后生成或刷新时使用的配置，不改变普通剑直接伤害。

正式游戏的 `SwordAbilityController.burn_config` 默认为空，因此玩家开局自带的是普通剑。选择局内卡牌 `剑:火焰附魔` 后，控制器才注入该配置；已经施加到敌人上的灼烧会按原剩余时间结束。

火焰剑现在已制作成一次性局内技能卡牌，真实 ID 为 `剑:火焰附魔`，上限 1 次，已加入正式升级池；选中后才解锁上述三条火焰强化，不写入局外存档。
...
三张历史截图已嵌入 [Word 手册附录 B](manuals/战斗实验室操作与实现手册.docx)。截图来自 2026-09-19 隔离展示场景；直接伤害配置为 5。截图证明火焰机制与表现曾被测试，不代表当前正式游戏开局自带火焰；当前需选择 `剑:火焰附魔` 卡牌后才生效。
...
2026-09-23 火焰剑接入正式升级卡牌后，在隔离副本补跑：火焰专项回归 34 项、0 失败；战斗实验室回归 269 项、0 失败，覆盖开局无火焰、正式升级池包含 `剑:火焰附魔`、选择卡牌后生效及一次性上限。2026-09-23 后续加入三条火焰强化线，隔离副本回归为 339 项、0 失败；覆盖三条路线解锁、满级数值和原有剑升级。战斗实验室本轮退出仍报告 ObjectDB 与 13 个资源未释放；火焰专项退出报告 7 个资源仍在使用。未重新进行配置页人工画面验收。
