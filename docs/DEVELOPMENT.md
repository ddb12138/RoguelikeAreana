> 历史文档：现行维护入口为 [development/DEVELOPMENT.md](development/DEVELOPMENT.md)。本文件保留迁移前记录，不作为最新现状依据。
...
根目录 `AGENTS.md` 是项目级协作约定，`docs/ARCHITECTURE.md` 是知识导航，本文件记录验证方式和现有问题。
...
| 缓存生成后再次导入 | 见本节下方补充记录 | 用于区分冷缓存现象与持续错误 |
...
| Buff 清理与层数（2026-09-19 部分修复） | BuffManager 已在过期时调用销毁并释放节点，灼烧采用单实例刷新；Player 初始化仍只使用 Buff key，不使用 quantity | 灼烧清理已专项验证；永久 Buff 层数仍待设计 |
...
新增可独立运行的 `test/fire_buff_regression.tscn`，未引入 GUT/GdUnit，也未将回归挂入正式主场景。headless 与 Forward+ 均通过 34 项检查；首次冷导入和退出资源诊断仍存在，详见 [火焰实现、运行方式与日志](FIRE_BUFF.md)。同时修复巫师 `disappear` 方法轨道的缺参调用，并将半血状态改为生命变化触发一次，以兼容周期伤害。
