...
2026-09-23 火焰剑正式接入一次性局内卡牌 `剑:火焰附魔`，开局普通剑仍不带火焰；选择卡牌后控制器复制 `sword_fire.tres`，并解锁灼烧伤害、持续时间、频率三条各 5 级强化线。隔离副本已通过战斗实验室回归 339 项、火焰专项回归 34 项，覆盖升级抽卡、三条路线满级数值、卡牌上限和火焰生效时机。退出仍有基线同类的对象／资源未释放诊断；未做配置页人工点击、完整通关或平台导出验证。
