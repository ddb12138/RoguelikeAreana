...
sences/                 原有目录拼写，所有主要场景与脚本
  ...
  buff/                 Buff 基类与治疗实现；灼烧实现与火焰视觉；冰目录目前只有图片
...
- 普通武器的 `HitboxComponent` 保存伤害值及可选 `burn_config`；敌人 `HurtboxComponent` 监听区域进入，调用生命组件并显示飘字、发出受击信号。
...
| 加 Buff | BuffBase → 实现与场景 → BuffManager；治疗走 buff_list，灼烧走 add_burn + BurnConfig；永久 Buff 再接 meta 资源与菜单 |
